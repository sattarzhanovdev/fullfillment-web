...
export default function FbsPickingPage() {
  ...
  const { data: items, isLoading } = useQuery({
    queryKey: ["picking-items"],
    queryFn: async () => (await apiClient.get<PickableItem[]>("/orders/picking/items")).data,
  ...
  const pickMutation = useMutation({
    ...
    onSuccess: async (_res, variables) => {
      ...
      playBeep(true);
      queryClient.invalidateQueries({ queryKey: ["picking-items"] });
  ...
  const notFoundMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["picking-items"] }),
  });
  ...
  return (
    <div onClick={() => inputRef.current?.focus()}>
      <PageHeader title="FBS · Сборка" description="Сканируйте товары для сборки заказов" />
      ...
      {isLoading ? (
        ...
        <EmptyState icon={ClipboardList} title="Нет товаров для сборки" description="Все заказы обработаны или нет активных заказов" />
      ) : (
